...
class Conv_3x1x3(nn.Module):
    def __init__(self, in_dim, out_dim, activation):
        super(Conv_3x1x3, self).__init__()
        self.conv1 = nn.Conv3d(in_dim, out_dim, kernel_size=(3, 1, 3), stride=1, padding=(1, 0, 1), bias=True)
        self.norm = SynchronizedBatchNorm3d(out_dim)
        self.act = activation
...
class HDC_block(nn.Module):
    def __init__(self, in_dim, out_dim, activation,flag):
        ...
        if flag:
            self.conv_1x3x3= Conv_3x1x3(out_dim, out_dim, activation)
        else:
...
class HDC_Net(nn.Module):
    def __init__(self, in_dim, out_dim, num_filters=32):
        ...
        self.conv_1 = HDC_block(self.n_f, self.n_f, self.activation,True)
        self.down_1 = Conv_down(self.n_f, self.n_f, self.activation)
        self.conv_2 = HDC_block(self.n_f, self.n_f, self.activation,False)
        self.down_2 = Conv_down(self.n_f, self.n_f, self.activation)
        self.conv_3 = HDC_block(self.n_f, self.n_f, self.activation,True)
        self.down_3 = Conv_down(self.n_f, self.n_f, self.activation)
        # bridge
        self.bridge = HDC_block(self.n_f, self.n_f, self.activation,False)
        # up
        self.up_1 = conv_trans_block_3d(self.n_f, self.n_f, self.activation)
        self.conv_4 = HDC_block(self.n_f * 2, self.n_f, self.activation,True)
        self.up_2 = conv_trans_block_3d(self.n_f, self.n_f, self.activation)
        self.conv_5 = HDC_block(self.n_f * 2, self.n_f, self.activation,False)
        self.up_3 = conv_trans_block_3d(self.n_f, self.n_f, self.activation)
        self.conv_6 = HDC_block(self.n_f * 2, self.n_f, self.activation,True)
